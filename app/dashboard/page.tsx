...
import { dataProduksi, getRekapBulanan } from "@/data/produksi"
import { rekapKeuangan, operasionalBulanan, pengeluaranKonstruksi, dataAyam, rupiah } from "@/data/dashboard"

// ─── Nav config ──────────────────────────────────────────────────────────────
const NAV = [
  { label: "Dashboard",  icon: "dashboard",   href: "/dashboard",  active: true },
  { label: "Production", icon: "egg",          href: "/produksi",   active: false },
  { label: "Finance",    icon: "payments",     href: "/finance",    active: false },
  { label: "Livestock",  icon: "pets",         href: "/livestock",  active: false },
  { label: "Inventory",  icon: "inventory_2",  href: "/inventory",  active: false },
]

// ─── Cashflow data (pendapatan vs biaya per bulan operasional) ────────────────
const cashflowData = [
  { label: "Feb",   income: 0,          expense: 9_575_000 },
  { label: "Mar",   income: 0,          expense: 13_260_000 },
  { label: "Apr",   income: 0,          expense: 14_063_000 },
  { label: "Mei",   income: 750_000,    expense: 15_115_000 },
  { label: "Jun",   income: 6_290_000,  expense: 15_045_000 },
  { label: "Jul",   income: 6_290_000,  expense: 15_045_000 },
  { label: "Ags",   income: 13_850_000, expense: 15_000_000 },
  { label: "Sep",   income: 11_124_000, expense: 15_000_000 },
  { label: "Okt",   income: 52_000_000, expense: 15_000_000 },
  { label: "Nov",   income: 51_500_000, expense: 15_000_000 },
  { label: "Des",   income: 52_050_000, expense: 15_000_000 },
]
...
const ALERTS = [
  {
    ...
    desc: `Pendapatan Rp189 jt vs investasi Rp482 jt. ROI ${((rekapKeuangan.pendapatanTelur / rekapKeuangan.totalInvestasi) * 100).toFixed(1)}% — butuh ±18 bulan lagi.`,
  ...
  },
  {
    ...
    desc: "Ayam batch Feb 2025 (326 ekor) jadwal booster bulan depan. Cek stok vaksin di gudang.",
    time: "1 jam lalu",
    type: "warning",
  },
  {
    ...
    desc: "HDP stabil di 51.4–51.5% sejak Oktober. Performa excellent — pertahankan pola pakan saat ini.",
    time: "4 jam lalu",
    type: "success",
  },
]
...
function GaugeChart({ value, max = 70 }: { value: number; max?: number }) {
  ...
  // show 75% of circle (270deg arc), starting from 135deg
...
export default function DashboardPage() {
  ...
  const latestHdp   = dataProduksi[dataProduksi.length - 1]?.hdp ?? 0
  const latestDay   = dataProduksi[dataProduksi.length - 1]
  const prevHdp     = dataProduksi[dataProduksi.length - 8]?.hdp ?? 0
  const hdpDiff     = +(latestHdp - prevHdp).toFixed(1)
  const peakHdp     = Math.max(...dataProduksi.map(d => d.hdp))
  const totalPendapatan = rekapKeuangan.pendapatanTelur + rekapKeuangan.penjualanAyam
  const totalBiayaOps = operasionalBulanan.reduce((a, b) => a + b.total, 0)
  const roiPercent = (rekapKeuangan.pendapatanTelur / rekapKeuangan.totalInvestasi) * 100

  // Dynamic alert generator based on real data conditions
  const generateDynamicAlerts = useCallback(() => {
    ...
    // Alert 3: Cost warning
    if (totalBiayaOps > 15_000_000) {
      alerts.push({
        title: "Biaya Operasional Tinggi",
        desc: `Pengeluaran operasional bulan ini Rp${(totalBiayaOps / 1_000_000).toFixed(1)}jt — cek efisiensi pembelian pakan dan utilitas.`,
        type: "warning",
        priority: 2,
      })
    }

    // Sort by priority (1 = highest) and return highest priority alert
    return alerts.sort((a, b) => a.priority - b.priority)[0]
  }, [hdpDiff, latestHdp, prevHdp, roiPercent, totalBiayaOps])

  // Auto-show dynamic alert whenever data changes
  useEffect(() => {
    const dynamicAlert = generateDynamicAlerts()
    if (dynamicAlert) {
      const icon = dynamicAlert.type === "error" ? "⚠️" : dynamicAlert.type === "warning" ? "⚡" : "✓"
      setTimeout(() => setToast({ msg: `${icon} ${dynamicAlert.title}`, type: dynamicAlert.type === "error" ? "alert" : (dynamicAlert.type as "success" | "warning") }), 0)
    }
  }, [generateDynamicAlerts])

  // HDP monthly avg
  const hdpBarData = rekap.map(r => ({
    label: r.bulan.slice(0, 3),
    hdp: r.avgHdp,
  }))

  // HDP weekly avg — group dataProduksi into 7-day buckets
  const hdpWeeklyData = useMemo(() => {
    const weeks: { label: string; hdp: number; min: number; max: number }[] = []
    const sorted = [...dataProduksi].sort((a, b) => a.date.localeCompare(b.date))
    ...
    <>
      <style>{`
        ...
        .chart-card::before {
          ...
          background: linear-gradient(to right, rgba(78,222,163,0.3), rgba(78,222,163,0.05), transparent);
        }
        .alert-item {
          padding: 16px;
          border-bottom: 1px solid rgba(60,74,66,0.5);
        ...
        @keyframes led-pulse {
          0%,100% { box-shadow: 0 0 4px rgba(255,180,171,0.5); opacity: 1; }
          50% { box-shadow: 0 0 12px rgba(255,180,171,0.9); opacity: 0.5; }
        }
      ...
      `}</style>
      {toast && <Toast msg={toast.msg} type={toast.type} onClose={() => setToast(null)} />}      <div style={{ display: "flex", minHeight: "100vh", background: "#050505", color: "#e5e2e1", fontFamily: "'Inter', sans-serif" }}>

        {/* ── Sidebar ── */}
        <aside style={{
          width: 256, flexShrink: 0, background: "#121212",
          borderRight: "1px solid #3c4a42",
          display: "flex", flexDirection: "column",
          padding: "24px 0", position: "sticky", top: 0, height: "100vh", zIndex: 50,
        }}>
          {/* Logo */}
        ...
        <main style={{ flex: 1, minHeight: "100vh", display: "flex", flexDirection: "column" }}>
          ...
          {/* TopBar */}
          <header style={{
            height: 64, background: "rgba(5,5,5,0.85)", backdropFilter: "blur(12px)",
            borderBottom: "1px solid #3c4a42",
            display: "flex", alignItems: "center", justifyContent: "space-between",
            padding: "0 24px", position: "sticky", top: 0, zIndex: 40,
          }}>
            <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
              <h1 style={{ fontSize: 22, fontWeight: 600, color: "#4edea3", margin: 0, letterSpacing: "-0.01em" }}>
                PoultryPro Analytics
              </h1>
            </div>
          ...
          <div style={{ padding: 24, maxWidth: 1600, margin: "0 auto", width: "100%" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 32 }}>
              ...
              {/* Total Eggs */}
              <div className="stat-card" style={{ borderColor: "#2E2E2E" }}
              ...
              >
                ...
                <p style={{ fontSize: 11, fontWeight: 700, color: "#bbcabf", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 4 }}>Telur Terakhir</p>
                <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                  <h2 style={{ fontSize: 36, fontWeight: 700, color: "#e5e2e1", margin: 0, lineHeight: 1 }}>{fmtInt(latestDay?.act ?? 0)}</h2>
                  <span style={{ fontSize: 13, fontWeight: 700, color: "#4edea3" }}>butir</span>
                </div>
                <div style={{ marginTop: 16, height: 4, background: "#2a2a2a", borderRadius: 2, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${Math.min(latestHdp, 100)}%`, background: "#4edea3", borderRadius: 2 }} />
                </div>
              </div>

              {/* HDP */}
              <div className="stat-card"
                onMouseEnter={e => (e.currentTarget.style.borderColor = "#89ceff")}
                onMouseLeave={e => (e.currentTarget.style.borderColor = "#2E2E2E")}
              >
              ...
              {/* Pendapatan */}
              <div className="stat-card"
                onMouseEnter={e => (e.currentTarget.style.borderColor = "#ffb95f")}
                onMouseLeave={e => (e.currentTarget.style.borderColor = "#2E2E2E")}
              >
                ...
                <div style={{ marginTop: 16, height: 4, background: "#2a2a2a", borderRadius: 2, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${(totalPendapatan / rekapKeuangan.totalInvestasi) * 100}%`, background: "#ffb95f", borderRadius: 2 }} />
                </div>
              </div>
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                  <h2 style={{ fontSize: 36, fontWeight: 700, color: "#e5e2e1", margin: 0, lineHeight: 1 }}>{fmtInt(dataAyam.aktif)}</h2>
                  <span style={{ fontSize: 13, fontWeight: 700, color: "#bbcabf" }}>ekor</span>
                </div>
                <div style={{ marginTop: 16, display: "flex", gap: 3 }}>
                  {[...Array(5)].map((_, i) => (
                    <div key={i} style={{ flex: 1, height: 4, background: i < dataAyam.batch.length ? "#4edea3" : "#2a2a2a", borderRadius: 2 }} />
                  ))}
                </div>
              </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(12,1fr)", gap: 16, marginBottom: 16 }}>
              ...
              <div className="chart-card" style={{ gridColumn: "span 8" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
                  <div>
                    ...
                    <p style={{ fontSize: 11, fontWeight: 700, color: "#bbcabf", margin: "4px 0 0", letterSpacing: "0.03em" }}>
                      {hdpView === "monthly"
                        ? `${rekap.length} bulan · Jul–Des 2025`
                        : `${hdpWeeklyData.length} minggu · Jul–Des 2025`}
                    </p>
                  </div>
                ...
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={activeHdpData} margin={{ top: 4, right: 0, left: -20, bottom: 0 }}>
                    ...
                    <XAxis dataKey="label" tick={{ fontSize: hdpView === "weekly" ? 9 : 11, fill: "#bbcabf" }} tickLine={false} axisLine={false}
                      interval={hdpView === "weekly" ? 2 : 0} />
                    ...
                    <Tooltip
                      content={({ active, payload, label }) => {
                        ...
                        return (
                          <div className="fc-tooltip">
                            <p style={{ color: "#bbcabf", marginBottom: 6, fontWeight: 600, fontSize: 12 }}>
                              {hdpView === "weekly" ? `Minggu ${label}` : label}
                            </p>
                            <div style={{ fontSize: 12, color: "#4edea3", fontWeight: 700 }}>
                              Avg HDP: {d?.hdp !== undefined ? d.hdp.toFixed(1) : "0.0"}%
                            </div>
                    ...
                    <Bar dataKey="hdp" radius={[4, 4, 0, 0]}>
                      {activeHdpData.map((d, i) => (
                        <Cell key={i}
                          fill={d.hdp >= 50 ? "#4edea3" : d.hdp >= 35 ? "#ffb95f" : "#ffb4ab"}
                          fillOpacity={0.85}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, fontSize: 11, fontWeight: 700, color: "#bbcabf", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                  {hdpView === "monthly" ? (
                    <>
                      <span>JUL 2025</span>
                      <span>SEPT 2025</span>
              ...
              <div className="chart-card" style={{ gridColumn: "span 4", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                ...
                <div style={{ marginTop: 16 }}>
                  <GaugeChart value={latestHdp} max={70} />
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 16 }}>
                  {[
                    { label: "Target", value: "50.0%" },
                    { label: `Peak (${dataProduksi.find(d => d.hdp === peakHdp)?.month ?? "Okt"})`, value: `${peakHdp.toFixed(1)}%` },
                    { label: "Ayam aktif", value: `${fmtInt(dataAyam.aktif)} ekor` },
                  ].map(item => (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(12,1fr)", gap: 16, marginBottom: 32 }}>
              ...
              <div className="chart-card" style={{ gridColumn: "span 7" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                  <div>
                    ...
                    <p style={{ fontSize: 11, fontWeight: 700, color: "#bbcabf", margin: "4px 0 0", letterSpacing: "0.03em" }}>Income vs Expense (per bulan)</p>
                  </div>
                  <span className="material-symbols-outlined" style={{ color: "#4edea3" }}>payments</span>
                </div>
                <ResponsiveContainer width="100%" height={180}>
                  <LineChart data={cashflowData} margin={{ top: 4, right: 4, left: -15, bottom: 0 }}>
                    <defs>
              ...
              {/* Critical Alerts */}
              <div style={{
                gridColumn: "span 5", background: "#121212", border: "1px solid #2E2E2E",
                ...
                <div style={{ padding: "16px 24px", borderBottom: "1px solid #3c4a42", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <span style={{
                    background: "#93000a", color: "#ffdad6", fontSize: 10, fontWeight: 700,
                    padding: "3px 8px", borderRadius: 20, letterSpacing: "0.04em",
                  }}>2 ACTION REQUIRED</span>
                </div>
                  ...
                  {ALERTS.map((alert, i) => (
                    <div key={i} className="alert-item">
                      <div style={{ display: "flex", gap: 12 }}>
                        <div style={{
                          width: 8, height: 8, borderRadius: "50%", background: alert.color,
                          marginTop: 6, flexShrink: 0,
                          ...(alert.pulse ? { animation: "led-pulse 2s infinite" } : {}),
                        }} />
                        <div style={{ flex: 1 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                            <span style={{ fontSize: 14, fontWeight: 600, color: "#e5e2e1" }}>{alert.title}</span>
                            <span style={{ fontSize: 10, color: "#bbcabf" }}>{alert.time}</span>
                          </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16, paddingBottom: 80 }}>
              <div className="chart-card" style={{ gridColumn: "span 2" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div style={{ background: "#1c1b1b", padding: 16, borderRadius: 8, border: "1px solid #3c4a42" }}>
                    ...
                    <div style={{ fontSize: 22, fontWeight: 700, color: "#4edea3" }}>{rupiah(rekapKeuangan.totalInvestasi, true)}</div>
                    <p style={{ fontSize: 11, fontWeight: 700, color: "#bbcabf", margin: "8px 0 0" }}>11 Construction Categories</p>
                  </div>
                  <div style={{ background: "#1c1b1b", padding: 16, borderRadius: 8, border: "1px solid #3c4a42" }}>
                    <p style={{ fontSize: 11, fontWeight: 700, color: "#bbcabf", margin: "0 0 8px", textTransform: "uppercase", letterSpacing: "0.04em" }}>Ayam Aktif</p>
                    <div style={{ fontSize: 22, fontWeight: 700, color: "#89ceff" }}>{((dataAyam.aktif / dataAyam.totalDibeli) * 100).toFixed(1)}%</div>
                    <p style={{ fontSize: 11, fontWeight: 700, color: "#bbcabf", margin: "8px 0 0" }}>{dataAyam.aktif} dari {dataAyam.totalDibeli} ekor</p>
                  </div>
                </div>
                {/* Top 5 konstruksi */}
                <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 8 }}>
                  {pengeluaranKonstruksi.slice(0, 4).map((item, i) => {
                    const colors = ["#4edea3", "#89ceff", "#ffb95f", "#ffb4ab"]
                    const pct = (item.jumlah / rekapKeuangan.totalInvestasi) * 100
                    return (
                      <div key={i}>
              ...
              <div className="chart-card" style={{
                ...
                <div style={{
                  ...
                  background: "radial-gradient(circle at 60% 40%, #4edea3 0%, transparent 65%)",
                  pointerEvents: "none",
                }} />
                <div style={{ position: "relative", zIndex: 1 }}>
                  ...
                  <p style={{ fontSize: 14, color: "#bbcabf", maxWidth: 280, margin: "0 auto 20px", lineHeight: 1.6 }}>
                    Import Excel berhasil dipetakan ke semua visualisasi secara real-time.
                  </p>
                  <button style={{
                    padding: "8px 24px", border: "1px solid #4edea3", color: "#4edea3",
                    background: "transparent", borderRadius: 8, cursor: "pointer", fontSize: 14,
                    transition: "background 0.15s",
                  }}
                    onMouseEnter={e => (e.currentTarget.style.background = "rgba(78,222,163,0.1)")}
                    onMouseLeave={e => (e.currentTarget.style.background = "transparent")}
                  >
                    Manage Data Source
                  </button>
        ...
        <nav style={{
          ...
          background: "#121212", borderTop: "1px solid #3c4a42",
          ...
          {[
            ...
            <a key={item.label} href="#" style={{
              ...
              textDecoration: "none", color: item.active ? "#4edea3" : "#bbcabf",
            }}>
